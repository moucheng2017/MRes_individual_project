...
for c=1:length(cases) 
    ...
    switch label
        case 'Healthy'
            folder_healthy = strcat('C:\Users\NeuroBeast\Desktop\',resolution,'_resolution_patches\patches_',patch_size_string,'\Healthy');
            for j=1:width
                for i =1:height
            ...
            %hold off
        case 'Tumour'
             folder_tumour = strcat('C:\Users\NeuroBeast\Desktop\',resolution,'_resolution_patches\patches_',patch_size_string,'\Tumour');
             for j=1:width
                for i =1:height
                    if (processed_truth(i,j,1)< r && processed_truth(i,j,2)>g && processed_truth(i,j,3)<b)
             ...
             for m=0:x_steps
                for n=0:y_steps
                    ...
                    t_file_name = sprintf('patch_tumour_%d_%d.jpg',patch_size,amount_patches_tumour);
                    fullname_t = fullfile(folder_tumour,t_file_name);
                    %save(fullname_t,'patch_tumour');
                    imwrite(patch_tumour,fullname_t);
             ...
             %hold off
        case 'Others'
            folder_others = strcat('C:\Users\NeuroBeast\Desktop\',resolution,'_resolution_patches\patches_',patch_size_string,'\Others');
            for j=1:width
                for i =1:height
                    if (processed_truth(i,j,1)< r && processed_truth(i,j,2)>g && processed_truth(i,j,3)>b)
